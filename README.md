...
REST API будет необходим для общения мобильного и веб приложений с сервисами поскольку:
 - необходима высокая скорость ответа
 - тип API удобен для реализации логики клиентской части
 - отсутствует большой поток событий
 - поддерживает кэширование


### AsyncAPI

AsyncAPI будет использоваться сервисов между собой и IoT устройствами, т.к.:
 - взаимодействия не требуют моментального ответа
 - устройства и сервисы создают большой поток сообщений

Для взаимодействия между сервисами будет использоваться Kafka, а для общения с IoT - легковесный MQTT брокер сообщений

В дальнейшем, например при появлении новой логики и требований, имеет смысл внедрить gRPC API. Он может использоваться для синхронного, но более быстрого общения между сервисами сценариев и устройств (для фоновых сценариев, а не вызываемых из клиента). Таким образорм при большом потоке вызовов все еще сохранится скорость по сравнению с REST API. 
...
Приложение temperature-api написано на Python с использованием Flask
